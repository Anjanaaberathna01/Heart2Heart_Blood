...
<head>
    ...
    <title>Hospital Locations Map - Admin</title>
...
<div class="container">
    <a href="{{ route('admin.dashboard') }}" class="back-btn"><i class="fas fa-arrow-left"></i> Back to Dashboard</a>

    <!-- Header -->
    <div class="header">
        <h1><i class="fas fa-map-location-dot"></i> Hospital Locations Map</h1>
        <p>View all registered hospitals on interactive map</p>
    </div>

    <!-- Statistics -->
    <div class="stats-grid">
        <div class="stat-card">
            <div class="number">{{ $hospitals->count() }}</div>
        ...
        <div class="stat-card">
            <div class="number">{{ $hospitals->where('available_for_donation', false)->count() }}</div>
            <div class="label">Unavailable Hospitals</div>
        </div>
        <div class="stat-card">
            <div class="number">{{ $hospitals->groupBy('district')->count() }}</div>
    ...
    <div class="hospitals-list">
        <h2><i class="fas fa-list"></i> Hospital Details</h2>
        @if($hospitals->isEmpty())
            <p style="color: #666;">No hospitals registered yet.</p>
        @else
            @foreach($hospitals as $hospital)
                <div class="hospital-item">
                    ...
                    <span class="badge {{ $hospital->available_for_donation ? 'badge-available' : 'badge-unavailable' }}">
                        {{ $hospital->available_for_donation ? '✓ Available' : '✗ Unavailable' }}
                    </span>
                </div>
            @endforeach
        @endif
...
<script>
    ...
    hospitals.forEach(hospital => {
        const coords = districtCoordinates[hospital.district] || [7.8731, 80.7718];

        // Green for available, Red for unavailable
        const markerColor = hospital.available_for_donation ? '#28a745' : '#dc3545';
        const status = hospital.available_for_donation ? 'AVAILABLE' : 'UNAVAILABLE';
        ...
        const popupContent = `
            <div style="font-family: Arial, sans-serif; width: 280px;">
                ...
                <p><strong>Status:</strong> <span style="color: ${markerColor}; font-weight: bold;">${status}</span></p>
                <p><strong>Hospital ID:</strong> ${hospital.hospital_id}</p>
                <p><strong>Email:</strong> ${hospital.email}</p>
                <p><strong>Phone:</strong> ${hospital.mobile_number1}${hospital.mobile_number2 ? ', ' + hospital.mobile_number2 : ''}</p>
                <p><strong>District:</strong> ${hospital.district}</p>
                <p><strong>Address:</strong> ${hospital.address}</p>
                <p><strong>Registered:</strong> ${new Date(hospital.created_at).toLocaleDateString()}</p>
            </div>
        `;
